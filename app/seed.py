...
def seed():
    """
    Popula o banco com dados iniciais para desenvolvimento e demonstração.
    Cria 2 usuários, 8 categorias e 12 transações distribuídas em março e abril.

    Execução segura — verifica se o banco já tem dados antes de inserir.
    Chamado automaticamente no startup da aplicação via main.py.
    """
    db = SessionLocal()

    # idempotente — se já tiver dados não faz nada
    ...
    # ── Usuários ──────────────────────────────────────
    # senhas hasheadas aqui — nunca armazena texto puro
    usuario1 = UserTable(
        nome="Admin",
    ...
    # refresh necessário para obter os ids gerados pelo banco
    # sem isso usuario1.id e usuario2.id seriam None
    db.refresh(usuario1)
    db.refresh(usuario2)

    # ── Categorias ────────────────────────────────────
    # cada usuário tem suas próprias categorias — isolamento garantido pelo user_id
    categorias_u1 = [
    ...
    # refresh em todas para ter os ids antes de criar as transações
    for cat in categorias_u1 + categorias_u2:
        db.refresh(cat)

    # ── Transações ────────────────────────────────────
    # índices dos arrays de categoria:
    # u1: [0]=Salário [1]=Freelance [2]=Alimentação [3]=Transporte [4]=Moradia
    # u2: [0]=Salário [1]=Alimentação [2]=Lazer
    transacoes = [
...
if __name__ == "__main__":
    # permite rodar o seed manualmente: python -m app.seed
    seed()